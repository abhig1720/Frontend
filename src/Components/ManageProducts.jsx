...
function ManageProducts() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      try {
        await axios.delete(`http://localhost:3002/products/delete/${id}`);
        fetchProducts();
      } catch (error) {
  ...
  const handleEdit = (product) => {
    ...
    setPreviewImage(`http://localhost:3002/uploads/${product.image}`);
    setEditModal(true);
  };
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:3002/products/${selectedProduct._id}`, data, {
        ...
        <table>
          ...
          <tbody>
            {products.map((product) => (
              <tr key={product._id}>
                <td><img src={`http://localhost:3002/uploads/${product.image}`} alt={product.name} width="50" /></td>
                <td>{product.name}</td>
