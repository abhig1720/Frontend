...
const Bag = () => {
  ...
  const handleUpdateQty = async (productId, newQty) => {
    await fetch(`http://localhost:3002/cart/updateCart/${user._id}/${productId}`, {
      method: 'PUT',
  ...
  const handleRemove = async (productId) => {
    await fetch(`http://localhost:3002/cart/removeFromCart/${user._id}/${productId}`, { method: 'DELETE' });
    updateCart();
  };
        ...
        <div className="bag-items">
          {cartItems.map((item) => (
            <div key={item.product._id} className="bag-item">
              <img src={`http://localhost:3002/uploads/${item.product.image}`} alt={item.product.name} />
