...
const Checkout = () => {
  ...
  const handleRazorpayPayment = async () => {
    try {
      const res = await fetch("http://localhost:3002/api/payment/create-order", {
        method: "POST",
        headers: {
  ...
  handler: async function (response) {
    ...
    const orderRes = await fetch("http://localhost:3002/orders/placeOrder", {
      method: "POST",
  ...
  const handleOrder = async (e) => {
    ...
    if (paymentMethod === "COD") {
      setLoading(true);
      try {
        const res = await fetch("http://localhost:3002/orders/placeOrder", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            userId: user._id,
